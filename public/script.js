...
async function loadGenres() {
    // Genre dari library filters atau hardcoded
    const genres = [
...
async function showHome(push = true) {
    ...
    // Fetch latest updates untuk homepage
    ...
    <section class="mb-12">
        ...
        <div class="flex overflow-x-auto gap-4 hide-scroll pb-4 -mx-4 px-4 md:mx-0 md:px-0">
            ${popularList.slice(0, 10).map(item => `
            ...
            onclick="showDetail('${item.slug}')">
                ...
                <div class="absolute bottom-0 left-0 p-3 z-20 w-full">
                    ...
                    <p class="text-amber-400 text-xs font-semibold mt-1">⭐ ${item.rating || 'N/A'}</p>
                </div>
            </div>
            `).join('')}
        </div>
...
async function showGenre(slug, page = 1) {
    resetNavs();
    setLoading();
    const data = await fetchAPI(`${API_BASE}/library?page=${page}&genre=${slug}`);
    if (!data || !data.komikList || data.komikList.length === 0) { 
        redirectTo404(); 
        return; 
    }
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        ...
        onclick="showDetail('${item.slug}')">
            ...
            <div class="p-3 text-center">
                ...
                <p class="text-[10px] text-amber-500 mt-1 font-medium">${item.chapters?.[0]?.title || item.latestChapter || 'Baca Sekarang'}</p>
            </div>
        </div>
        `).join('')}
    </div>
...
async function showDetail(idOrSlug, push = true) {
    ...
    // Get status from detail object
    const status = res.detail?.status || res.status || 'Unknown';
    const type = res.detail?.type || res.type || 'Comic';
    const rating = res.rating || 'N/A';
    ...
    <div class="relative z-10 flex flex-col md:flex-row gap-8 lg:gap-12 mt-4 animate-fade-in">
        <div class="md:w-[280px] flex-shrink-0 mx-auto md:mx-0 w-full max-w-[280px]">
            <div class="relative group">
                <span class="type-badge ${getTypeClass(type)} scale-110 top-4 left-4 shadow-lg">${type}</span>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-3 mb-6">
                ...
                <div class="glass px-4 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold text-green-400 border border-green-500/20">
                    <i class="fa fa-circle text-[6px]"></i> ${status}
                </div>
            ...
            <div class="flex flex-wrap gap-2 mb-6">
                ...
                <span onclick="showGenre('${g.slug.replace('/genres/', '')}')" class="cursor-pointer hover:text-amber-500 transition text-gray-400 text-xs px-3 py-1 rounded-full border border-white/10 hover:border-amber-500/50 bg-white/5">
                    ${g.name || g.title}
                </span>`).join('') : ''}
            </div>
...
async function readChapter(chIdOrSlug, comicSlug = null, push = true) {
    ...
    try {
        ...
        if (!finalComicSlug) {
            ...
            else if (res.komikInfo?.slug) finalComicSlug = res.komikInfo.slug;
        }
        
        const comicTitle = currentComicContext?.title || res.komikInfo?.title || res.thumbnail?.title || "Komik";
...
function showHistory() {
    ...
    resetNavs();
    let history = JSON.parse(localStorage.getItem('fmc_history') || '[]');
    renderGrid({ komikList: history }, "Riwayat Baca", null);
}
...
function showBookmarks() {
    ...
    resetNavs();
    let bookmarks = JSON.parse(localStorage.getItem('fmc_bookmarks') || '[]');
    renderGrid({ komikList: bookmarks }, "Koleksi Favorit", null);
}
